...
// After your existing "renderClues()" call, add a new listener
document.getElementById("submit-all").addEventListener("click", checkAllAnswers);

// This function checks every input box at once
function checkAllAnswers() {
  ...
  inputs.forEach((input, i) => {
    const correctAnswer = words[i].answer; // words[i] matches the same index
    const userAnswer = input.value.toUpperCase();
  ...
  // Show a message depending on results
  if (hasWrong) {
    messageElement.textContent = "Some answers are incorrect! Try again or check the clues.";
  } else {
    messageElement.textContent = ""; // or do nothing, because we'll handle final success below
    checkCompletion(); // calls your existing "displayCompletionMessage()" if all are correct
  }
}
